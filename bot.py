#!/usr/bin/env python3
"""
🎵 Melody Stream Bot - Advanced Telegram Music Bot (PRO FX)
Powered by JioSaavn
Created by: Aliya (@reedagotsidd)
"""
...
# Bot Configuration
BOT_TOKEN = "8703222748:AAEHSx9K1iM5M05w25tv5YpfxwgII-u4E6g"
ADMIN_ID = 8833921540
API_BASE_URL = "https://jiosavanapiryden.vercel.app/api"
SUPPORT_LINK = "https://t.me/reedagotsidd"
CHANNEL_LINK = "https://t.me/+OIakUpUThjE3MGFl"
# Placeholder for group welcome video (add a valid Telegram file_id if desired)
GROUP_WELCOME_VIDEO_ID = ""  # e.g., "BAACAgIAAxkBAA..."
...
def on_new_chat_members(message):
    """Send welcome message/video when the bot is added to a group."""
    for member in message.new_chat_members:
        if member.id == bot.get_me().id:
            # Bot added to group
            if GROUP_WELCOME_VIDEO_ID:
                try:
                    bot.send_video(
                        message.chat.id,
                        GROUP_WELCOME_VIDEO_ID,
                        caption=style_primary("🎵 Welcome to Melody Stream Pro!\nSend `find song name` to download music."),
                        parse_mode="Markdown"
                    )
                except Exception as e:
                    print(f"Failed to send welcome video: {e}")
                    fallback_text = style_primary("🎵 Welcome to Melody Stream Pro!\n\nSend `find song name` to download high‑quality music.\nJoin @reedagotsidd for updates.")
                    bot.send_message(message.chat.id, fallback_text, parse_mode="Markdown")
            else:
                welcome_text = style_primary("🎵 Welcome to Melody Stream Pro!\n\nSend `find song name` to download high‑quality music.\n\n👨‍💻 Developer: Aliya (@reedagotsidd)")
                bot.send_message(message.chat.id, welcome_text, parse_mode="Markdown")

            # Optional: send an inline menu
            markup = types.InlineKeyboardMarkup(row_width=2)
            markup.add(
                types.InlineKeyboardButton("🎵 Search Music", callback_data="quick_search"),
                types.InlineKeyboardButton("📖 Help", callback_data="quick_help")
            )
            markup.add(
                types.InlineKeyboardButton("📢 Channel", url=CHANNEL_LINK),
                types.InlineKeyboardButton("👨‍💻 Developer", url=SUPPORT_LINK)
            )
            bot.send_message(message.chat.id, style_primary("Use the buttons below:"), reply_markup=markup)
            break
...
━━━━━━━━━━━━━━━━
🆘 NEED HELP? Contact: @reedagotsidd
...
def handle_text(message):
    ...
    # Only respond to messages starting with "find "
    if text_lower.startswith("find "):
        ...
        # No animation to keep it fast
        bot.edit_message_text(style_primary(f"🔍 Looking for '{search_query}'..."), message.chat.id, searching_msg.message_id)
    ...
    # Ignore all other text messages (no response)
    # The bot remains silent unless the message starts with 'find' or is a command/button
    return
...
def song_callback(call):
    ...
    # No animation, just static processing message
    processing_msg = bot.send_message(call.message.chat.id, style_primary("🎵 PROCESSING PRO FRAME... Please wait."))
    try:
        bot.edit_message_text(style_primary("⏳ Fetching high-quality audio..."), call.message.chat.id, processing_msg.message_id)
    except:
        pass
    ...
    try:
        ...
        bot.edit_message_text(style_primary(f"🎵 DOWNLOADING\n\n📀 Song: {title}\n🎤 Artist: {artist}\n\n⏳ Preparing audio..."), call.message.chat.id, processing_msg.message_id)
        ...
        bot.edit_message_text(style_primary(f"📤 Sending {title}..."), call.message.chat.id, processing_msg.message_id)
...
@bot.callback_query_handler(func=lambda call: call.data == "quick_help")
def quick_help_callback(call):
    bot.answer_callback_query(call.id, style_primary("📖 Sending help..."))
...
def main():
    ...
    print("=" * 50)
    print(f"👑 Developer: Aliya (@reedagotsidd)")
